...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =>
    {
        options.Cookie.SameSite = SameSiteMode.None;//frontend and backend are on same domains
    })
    .AddOpenIdConnect(options =>
    {
        ...
        options.CallbackPath = new PathString(azureSettings.ReturnUrl); // Needs to be registered as callback url by identity provider
        options.Scope.Clear();
...
{
    MinimumSameSitePolicy = SameSiteMode.None //frontend and backend are on same domains
    ,
